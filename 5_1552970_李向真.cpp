...
void split_line( const string & str, vector<string> & words )
{
    ...
    while( i != str.size() )
    {
        ...
        if( i != j )
        {
            words.push_back(str.substr( i, j - i ));  //将单词记录进数组
            i = j;
        }
    }
}
...
void word_analyse(string f,string word,vector<vector<int> >& ans){
    ...
    while (getline(file,line)){
        ...
        while(beg != string::npos){ //循环查找该行中该单词的所有出现情况
            ...
            if(beg != string::npos){
                pos += beg;
                positions.push_back(pos); //记录下每个出现位置
                pos += word.size();
            }
        }
        ans.push_back(positions);    //将该行的出现位置记录
    }
}
